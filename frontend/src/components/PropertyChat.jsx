...
export default function PropertyChat({ propertyId, sellerId, onClose }) {
  const { user } = useAuth();
  ...
  if (!user) {
    return (
      <div className="fixed bottom-6 right-6 w-96 h-[500px] rounded-2xl border-2 border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 shadow-2xl z-50 flex flex-col">
        <div className="p-4 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between">
          <h3 className="font-bold text-slate-900 dark:text-white">Property Chat</h3>
        ...
        <div className="flex-1 flex items-center justify-center p-4">
          <p className="text-slate-600 dark:text-slate-400 text-center">
            Please <a href="/login" className="text-slate-900 dark:text-white underline">login</a> to start chatting
          </p>
        </div>
      </div>
    );
  ...
  return (
    <div className="fixed bottom-6 right-6 w-96 h-[500px] rounded-2xl border-2 border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 shadow-2xl z-50 flex flex-col">
      {/* Header */}
      <div className="p-4 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between bg-gradient-to-r from-slate-900 to-slate-700 text-white">
        <div>
          <h3 className="font-bold">Property Chat</h3>
          <p className="text-xs opacity-80">Discuss this property</p>
        </div>
